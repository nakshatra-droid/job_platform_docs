...
| **Job Search** | Search by job by title and companies. |
| **Job Applications** | Apply for jobs with resume uploads. |
| **Profile Management** | Update user or employer details. |
| **Job Management** | Employers can post or delete jobs. |
| **File Uploads** | Resumes can be uploaded of seekers. |
...
- Scalable architecture deployable on **AWS and Vercel**.
...
| Feature | Description |
|----------|-------------|
| **Admin Dashboard** | A centralized admin portal to manage everything. |
| **Email / SMS Notifications** | Notify users on new jobs and applications. |
| **Cloud based file storage** | Store resumes on S3. |
| **OTP based verification** | Verify every user with OTP. |

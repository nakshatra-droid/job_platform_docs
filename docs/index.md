# Joblelo Documentation

Welcome to the **Joblelo Project Documentation** — a comprehensive guide to the design, development, and functionality of the Joblelo web application.

---

## Introduction

The **Joblelo Platform** is a full-stack web application designed to simplify the hiring and job-seeking process by bridging the gap between **job seekers** and **employers**.  

#### It enables:
- Job seekers to find and apply for jobs easily.  
- Employers to create, manage, and track job postings efficiently.  

It is built using **React.js**, **Node.js**, **Express.js**, and **PostgreSQL**, with **Sequelize** as ORM.  

This project represents a real-world implementation of the above **Tech stack** with cloud integration and responsive design principles.

---

## Project Overview

The system is divided into two main portals, each with specific responsibilities and restricted access:

### Job Seeker Portal
- Register and log in securely.  
- Browse and search for job opportunities.  
- Apply for jobs.  
- Manage profile.  
- Track applications.  
- Logout securely.

### Employer Portal
- Register and log in securely.  
- Manage job postings.  
- View all job applications for posted jobs.  
- Update profile and details.  
- Logout securely.

---

## Documentation Access

This documentation includes the following sections:

- **[Functional Documentation](functional_documentation.md)** — Explains the system’s features, objectives, scope, and architecture.

- **[Use Case Documentation](usecase_documentation.md)** — Details user–system interactions, use case tables, and flow diagrams.

---

## Summary

This documentation provides a complete understanding of the **Job Platform** project.  
It defines its **purpose**, **functionalities** and **user cases** — making it an ideal reference for both evaluators and developers.  
Proceed to explore the next sections to gain detailed insight into the system’s functional and use case perspectives.
